    ...
    <section id="precos" className="py-24 bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-20">
          ...
          >
            Planos que crescem <br /> com a sua empresa
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            Soluções a partir de 10 colaboradores, com estrutura escalável. Ideal para pequenas, médias e grandes empresas.
          </motion.p>
        </div>
          ...
          {plans.map((plan, i) => (
            ...
            >
              ...
              <div className="mb-4">
                ...
                <p className={`text-sm h-12 ${plan.featured ? "text-slate-300" : "text-slate-500"}`}>{plan.desc}</p>
              </div>
              
              <ul className="space-y-4 mb-8">
                {plan.features.map((feat, idx) => (
                  <li key={idx} className="flex items-center gap-3">
                    <Check className={`w-5 h-5 ${plan.featured ? "text-blue-400" : "text-slate-800"}`} />
                    <span className={`text-sm ${plan.featured ? "text-slate-200" : "text-slate-600"}`}>{feat}</span>
                  </li>
                ))}
              </ul>
              ...
              <button className={`w-full py-4 rounded-xl font-bold transition-all ${
                ...
                {plan.name === "Enterprise" ? "Falar com Consultor" : "Solicitar Proposta"}
              </button>
            </motion.div>
          ))}
        </div>
