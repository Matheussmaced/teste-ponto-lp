  ...
  return (
    <section className="py-24 bg-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        >
          Controle inteligente <br /> do início ao fim.
        </motion.h2>
        <motion.p
           initial={{ opacity: 0, y: 20 }}
           whileInView={{ opacity: 1, y: 0 }}
        ...
        <div className="space-y-12 md:space-y-0">
          {steps.map((step, i) => (
            <div key={i} className={`relative flex flex-col md:flex-row items-center ${i % 2 !== 0 ? 'md:flex-row-reverse' : ''}`}>
              ...
              {/* Dot mapping to line */}
              <div className="absolute left-[20px] md:left-1/2 w-3 h-3 bg-slate-800 rounded-full transform -translate-x-1/2 mt-6 md:mt-0 z-10 ring-4 ring-white"></div>
              ...
              <div className="w-full md:w-1/2 flex">
                ...
                >
                  <div className={`bg-white rounded-3xl p-8 border border-slate-100 shadow-xl shadow-slate-200/40 w-full md:max-w-[400px] text-left ${i % 2 !== 0 ? 'md:text-left' : 'md:text-right text-left'}`}>
                    <span className="text-xs font-bold tracking-widest text-slate-400 uppercase mb-3 block">{step.phase}</span>
